...
const MyAppointments = () => {
  ...
  return (
    <div>
      <p className='pb-3 mt-12 font-medium text-title border-b border-black'>My Appointment</p>
      <div>
        {appointments.map((item,index)=>(
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b border-gray-400' key={index}>

            <div>
              <img onClick={()=>{navigate(`/appointment/${item._id}`);scrollTo(0,0)}} className='w-32 bg-indigo-50 cursor-pointer' src={item.docData.image} alt="" />
            </div>

            <div className='flex-1 text-sm text-zinc-100'>
              <p className='text-title font-semibold'>{item.docData.name}</p>
              <p>{item.docData.speciality}</p>
              <p className='text-title font-medium mt-1'>Address:</p>
              <p className='text-xs' >{item.docData.address.line1}</p>
              <p className='text-xs'>{item.docData.address.line2}</p>
              <p className='text-xs mt-1'><span className='text-sm text-title font-medium'>Date & Time </span> {slotDateFormat(item.slotDate)} |{item.slotTime}</p>
